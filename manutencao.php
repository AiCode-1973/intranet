...
<body class="bg-background text-text font-sans selection:bg-primary/20 text-[13px]">
    ...
    <div id="modalManutencao" class="modal">
        <div class="bg-white w-full max-w-md mx-4 rounded-xl shadow-2xl border border-border overflow-hidden animate-in zoom-in duration-150">
            ...
            <form id="formNovoChamado" method="POST" action="" enctype="multipart/form-data" class="p-5">
    ...
    <div id="modalDetalhes" class="modal">
        <div class="bg-white w-full max-w-4xl mx-4 rounded-xl shadow-2xl border border-border overflow-hidden flex flex-col md:flex-row" style="height:58vh;max-height:58vh">
    ...
    <script>
        ...
        function verDetalhes(chamado) {
            ...
            // Chat
            const chatBox = document.getElementById('chat_mensagens');
            chatBox.innerHTML = '';

            if (chamado.comentarios && chamado.comentarios.length > 0) {
                chamado.comentarios.forEach(c => {
                    const isMe = (c.usuario_id == usuarioAtualId);
                    ...
                    if (c.anexo) {
                        anexoHtml = `
                            <a href="uploads/manutencao/${c.anexo}" target="_blank" class="mt-2 block rounded-lg overflow-hidden border border-border hover:opacity-80 transition-opacity">
                                <img src="uploads/manutencao/${c.anexo}" class="max-w-[150px] h-auto object-cover" onerror="this.src='imagens/file-icon.png'">
                            </a>
                        `;
                    }
                ...
                chatBox.innerHTML = `
                    <div class="flex flex-col items-center justify-center h-full opacity-20 py-10">
                        ...
                        <p class="text-[10px] font-black uppercase tracking-widest text-center">Nenhuma mensagem<br>nesta O.S.</p>
                    </div>
                `;
            }
            ...
            // Marcar como lido se houver novidade
            if (chamado.tem_novidade) {
                fetch('manutencao.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                    body: `acao=marcar_lido&manutencao_id=${chamado.id}`
                });
            }
        }

        // Upload Preview
        document.getElementById('anexo_input').addEventListener('change', function(e) {
            const fileName = e.target.files[0]?.name || "Clique para selecionar foto ou PDF";
            document.getElementById('file_name').innerText = fileName;
        });

        document.getElementById('anexo_com_input').addEventListener('change', function(e) {
